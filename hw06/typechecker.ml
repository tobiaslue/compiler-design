...
let rec typecheck_exp (c : Tctxt.t) (e : Ast.exp node) : Ast.ty =
  ...
  | Call (f, args) ->
      ...
      | TRef (RFun (l, RetVal r)) ->
          ...
          else List.iter2 (fun arg l -> if not (subtype c arg l) then type_error e "Incorrect type of argument") argtyps l;
          r
...
let rec typecheck_stmt (tc : Tctxt.t) (s:Ast.stmt node) (to_ret:ret_ty) : Tctxt.t * bool =
  ...
  | Assn (e1, e2) ->
    ...
    if subtype tc assn_from assn_to then tc, false else type_error s "Mismatched types in assignment"
  ...
  | SCall (f, args) -> 
      ...
      | TNullRef (RFun (l, RetVoid)) | TRef (RFun (l, RetVoid)) ->
          ...
          else List.iter2 (fun arg l -> if not (subtype tc arg l) then type_error s "Incorrect type of argument") argtyps l;
          tc, false
    ...
    begin match exp_type with
      | TNullRef r' ->
        if subtype_ref tc r' r then
          let (_, lft_ret) = typecheck_block (Tctxt.add_local tc id (TRef r)) b1 to_ret in
          let (_, rgt_ret) = typecheck_block tc b2 to_ret in
          tc, lft_ret && rgt_ret
        else
...
let create_function_ctxt (tc:Tctxt.t) (p:Ast.prog) : Tctxt.t =
  let builtins_context = 
    List.fold_left (fun c (id, (args, ret)) -> Tctxt.add_global c id (TRef (RFun(args,ret)))) tc builtins
  in
    List.fold_left (fun c d ->
      match d with
      | Gfdecl ({elt=f} as l)  ->
